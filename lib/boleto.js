...
//type 1 = calcular cod de barras
//type 2 = usar cod de barras pronto
var Boleto = function (options, type) {
  if (!options) {
  ...
  if (!options['data_vencimento']) {
    ...
    options['data_vencimento'] = moment(moment(options['data_vencimento']).utc().format('DD/MM/YYYY'))
  }
...
Boleto.prototype._calculate = function (type) {
  //construindo cod de barras
  if (type == 1) {
    this['codigo_banco'] = this.bank.options.codigo + '-' + formatters.mod11(this.bank.options.codigo)
    //criar funcao do digito verificador certo e inserir aqui
    ...
    this['codigo_banco'] = this.bank.options.codigo + '-' + formatters.mod11(this.bank.options.codigo)
    //criar funcao do digito verificador certo e inserir aqui
    this['barcode_data'] = this['codigo_barras']
    this['linha_digitavel'] = this['numero_documento']
  }
}
...
Boleto.prototype.renderHTML = function (callback) {
  ...
  //mudar detalhes do layout:
  //---0 a mais no NOSSO_NUMERO
  //---adicionar lugar pros campos de cota, grupo, numero e horario assembleia
...
//mesma função que a renderHTML mas em modo de return, e não callback
Boleto.prototype.returnRenderedHTML = function () {
  var self = this
  ...
  return ejs.renderFile(path.join(__dirname, '/../assets/layout.ejs'), renderOptions, {
    cache: true
  }, function (err, html) {
    if (err) {
      console.log(err)
